...
?>
 ...
 <!-- KLIENCI-->
  ...
  <tbody>
	...
	$query = sprintf("SELECT uzytkownicy.login ,rejestracja,email,ip,imie,nazwisko,adres FROM `uzytkownicy` LEFT JOIN dane on uzytkownicy.login = dane.login WHERE aktywacja = '0'");
	...
	echo "<td>".$row['ip']. "</td>"; 
	echo '<td><form role="form" method="POST" action="aktywacje-cp.php"><button type="submit" name="aktywacja1" class="btn btn-outline-danger"';
	echo " value=".$row['login'].">Aktywuj</button></form></td>"; 
...
<?php
if (isset($_POST['aktywacja1'])) 
		{
			 $value = $_POST['aktywacja1'];
			
					mysql_query("UPDATE `uzytkownicy` SET `aktywacja` = '1' WHERE login = '".$value."';");
