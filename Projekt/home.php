...
$query = sprintf("SELECT imie, nazwisko FROM dane WHERE login = '%s'",
...
?>
		   ...
		   <div class="col-lg-6">
		             <div class="card mb-3 bg-light">
            <div class="card-header">
              <i class="fa"></i>Saldo:</div>
			  <div class="card-body">
			  <?php echo "<h2>".$saldo." PLN</h2>"; ?>
			  </div>
			  </div>
		   </div>
	</div>
...
<div class="collapse" id="collapseExample">
  ...
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">Dane nadawcy / odbiorcy</th>
  ...
  <tbody>
	...
	$query = sprintf("SELECT data,dane,numer_konta,tytul,kwota FROM przelewy WHERE login = '%s' ORDER BY `id` DESC",
	mysql_real_escape_string($_SESSION['login']));
	$result = mysql_query($query);
    ...
    echo "<th>".$row['data']. "</th>";  
	echo "<td>".$row['dane']. "</td>"; 
	echo "<td>".$row['numer_konta']. "</td>"; 
	echo "<td>".$row['tytul']. "</td>"; 
	//echo "<td>".$row['kwota']. "</td>"; 
	 if($row['kwota']>0)
		echo '<td><p class="text-success">'.$row['kwota'].'</p></td>';
	else
		echo '<td><p class="text-danger">'.$row['kwota'].'</p></td>';
